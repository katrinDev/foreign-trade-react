...
export function ItemsGrid() {
  ...
  const handleEditRowsModelChange = (model) => {
    // update the rows state with the new values from the model
    setRows((state) =>
      state.map((row) => {
        const newRow = { ...row };
        Object.keys(model[row.id] || {}).forEach((field) => {
          newRow[field] = model[row.id][field].value;
        });
        return newRow;
      })
    );

    // call processRowUpdate with the updated row data
    Object.keys(model).forEach((id) => {
      const updatedRow = rows.find((row) => row.id === parseInt(id));
      if (updatedRow) {
